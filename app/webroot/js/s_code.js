...
var s_code_version="H25|20121126"

var s_domain = window.location.hostname;
 
switch(s_domain)
{
case "localhost":
  var s_account="amfamdev,amfamamfamcomdev"
  break;
case "www.amfam.com":
  var s_account="amfamprod,amfamamfamcomprod"
  break;
case "wwwd1.amfam.com":
  var s_account="amfamdev,amfamamfamcomdev"
  break;
case "wwwa.amfam.com":
  var s_account="amfamdev,amfamamfamcomdev"
  break;
case "wwwqa.amfam.com":
  var s_account="amfamdev,amfamamfamcomdev"
  break;
case "www.amfamlatino.com":
  var s_account="amfamprod,amfamamfamlatinocomprod"
  break;
case "m.amfam.com":
  var s_account="amfammobileprod"
  break;
case "m.amfamlatino.com":
  var s_account="amfammobileprod"
  break;
case "mqa.amfam.com":
  var s_account="amfamamfammobiledev"
  break;
case "insurance-agency.amfam.com":
  var s_account="amfamprod,amfamfieldforceprod"
  break;
case "district-manager.amfam.com":
  var s_account="amfamprod,amfamfieldforcemgtprod"
  break;
case "insurance-agencyqa.amfam.com":
  var s_account="amfamdev,amfamfieldforcedev"
  break;
case "district-managerqa.amfam.com":
  var s_account="amfamdev,amfamfieldforcemgtdev"
  break;
case "insurance-agencypr.amfam.com":
  var s_account="amfamdev,amfamfieldforcedev"
  break;
case "district-managerpr.amfam.com":
  var s_account="amfamdev,amfamfieldforcemgtdev"
  break; 
  case "insurance-agency.amfamlatino.com":
  var s_account="amfamprod,amfamamfamlatinocomprod,amfamfieldforceprod"
  break; 
default:
  var s_account="amfamprod,amfamamfamcomprod"
}
...
s.linkInternalFilters="javascript:,amfam.com,amfamlatino.com,teensafedriver.com,amfambusinessaccelerator.com,buildingabrighterfuture.msn.com,longliveourdreams.com,quevivanlossuenos.com,recruitmax.com,amfamresearch.com,amfamprotect.com"
...
s.prop50=s_code_version;

/* DART plugin config */
s.variableProvider = 'DFA#1516847:v14=[["DFA-"+lis+"-"+lip+"-"+lastimp+"-"+lastimptime+"-"+lcs+"-"+lcp+"-"+lastclk+"-"+lastclktime]]';
...
function s_doPlugins(s) {	
	...
	/* SV 6.8.11 if multiple ? are present in the URL, replace all instances after the first instance with & to avoid breaking campaign tracking. */
	var s_qs=s.split(document.location.toString(),"?");
	var s_newURL="";
	if(s_qs.length>2){
		s_newURL=s_qs[0]+"?";
		for(i=1;i<s_qs.length;i++){
			s_newURL+=s_qs[i]+"&";
		}
	}

	/* Internal Search */
	s.prop1=s.eVar1=s.getQueryParam('q').toLowerCase();
	
	/* redirects */
	s.prop11=s.getQueryParam('rd');

	/* External Campaign Tracking */
	if(!s.campaign){s.campaign=s.getQueryParam('s_cid,sourceid,cid,rmid','',s_newURL?s_newURL:s.wd.location); }
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	if(s.campaign.toLowerCase()=="null"){s.campaign="";}
	if(s.campaign){s.eVar35=s.crossVisitParticipation(s.campaign,"s_cvp",30,5,">",'',0);}
	
	/* Internal Campaign Tracking */
	s.eVar7=s.getQueryParam('tid');
	s.eVar7=s.getValOnce(s.eVar7,'s_ev7',0);
		
	/* Plugin Example: timeparting EST */
	s.prop4=s.eVar5=s.getTimeParting('h','-6'); // Set hour 
	s.prop3=s.eVar4=s.getTimeParting('d','-6'); // Set day
	...
	if(s.pageType=="errorPage"){
		s.pageName="404:"+s.wd.location.toString();
	}
	...
	if(s.events){
		if((s.events+",").indexOf("event37,")>-1 || s.events.indexOf("event37:")>-1)s.prop21="start";
		if((s.events+",").indexOf("event5,")>-1 || s.events.indexOf("event5:")>-1)s.prop21="stop";
		s.prop21=s.getTimeToComplete(s.prop21,'s_ttc',0);
	}
	
	/* file downloads conversion tracking */
	var linkObject=s.downloadLinkHandler('','true');
	if(linkObject){
		s.eVar54=linkObject.href;
		s.events="event35";
	...
	if (s.eVar3 && !s.transactionID){
		s.transactionID=s.eVar3;
	}
		
	/* T&T integration */
	s.tnt=s.trackTNT();
	
	/* Social Influence and Segmentation*/
	var linkObject=s.linkHandler('facebook.com/share.php|twitter.com/home|digg.com/submit|del.icio.us/post|stumbleupon.com/submit|reddit.com/submit|technorati.com/faves|plus.google.com/share|pinterest.com/pin/create/button|linkedin.com/shareArticle','o','true');
	if(linkObject){
		s.events="event60";
		s.socialshr=linkObject.href;
		s.linkTrackVars="events,eVar60,eVar61,eVar62";
		s.linkTrackEvents="event60";
	} 

	var vv = s.smSux('s_seg',6);
	if(vv){
		s.eVar60 = vv.getMostRecent(); 	//last social touchpoint
		s.eVar61 = vv.getAsStack("|"); 	//sequential social touchpoints up to 6 most recent
		s.eVar62 = vv.getAsStack("|",1)	//alphabetical last 6 social touchpoints
		s.eVar63 = s.getQueryParam('soc');
		s.eVar63 = s.getValOnce(s.eVar63,'s_ev63',0);
	};
	
	/* Adobe Social Integration Plugins */
	s.socialPlatforms('eVar57');
}
s.doPlugins=s_doPlugins

/* Adobe Social Authors plugin */
...
+"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar56';s.Integrate"
...
+"return 0;");
/*
* Plugin:  socialSegmentation v1.1
*/
s.smSuxConfig = {
	fbsh	:['Facebook',			['events','event60','socialshr','facebook.com/share.php']],
	twsh	:['Twitter',			['events','event60','socialshr','twitter.com/home']],
	dgsh	:['Digg',				['events','event60','socialshr','digg.com/submit']],
	dlsh	:['Delicious',			['events','event60','socialshr','del.icio.us/post']],
	sush	:['StumbleUpon',		['events','event60','socialshr','stumbleupon.com/submit']],
	rdsh	:['Reddit',				['events','event60','socialshr','reddit.com/submit']],
	tcsh	:['Technorati',			['events','event60','socialshr','technorati.com/faves']],
	gopl	:['Google',				['events','event60','socialshr','plus.google.com/share']],
	pint	:['Pinterest',			['events','event60','socialshr','pinterest.com/pin/create/button/']],
	lnkd	:['LinkedIn',			['events','event60','socialshr','linkedin.com/shareArticle']],
...
s.smSuxRules = {
	v:[function(u,y) {//eVar Values
		if (s[u[0]] == u[1] && s[u[2]].indexOf(u[3])!=-1){return 1;};
	},'SocialShare'],
	d:[function(u,y) { 
		if(typeof u != 'string') return;
		if(window!=window.top){ //if this is an iframe 
		var h = s.referrer ? s.referrer : document.referrer;
		return h.toString().indexOf(u);}
	},'AppInteraction'],
	r:[function(u,y) { 
		if(typeof u != 'string') return;
		if(window == window.top){//if this is not an iframe
		var h = s.referrer ? s.referrer : document.referrer;
		return h.toString().indexOf(u);}
	},'Referrer']	
};

s.smSux = new Function("cn","cl",""
+"var s=this;var cn=cn;var e=new Date();e.setFullYear(e.getFullYear()+"
+"5);var o=new Object;var b=new Object();var p=false;var dt=new Functi"
+"on(\"return \"+'{' + s.c_r(cn)+'}')();for(var i in s.smSuxRules) {fo"
+"r(var y in s.smSuxConfig) {if(s.smSuxRules[i][0](s.smSuxConfig[y][1]"
+",s.smSuxConfig[y][0])>-1){if(dt[y]){dt[y][i]=new Date().getTime();}e"
+"lse{o[i]=new Date().getTime();dt[y]=o;}p=true;}if(p) break;}if(p) br"
+"eak;}for(var f in dt){if(dt[f].length==0)delete dt[f];}var vv=new Ob"
+"ject();for(var f in dt){for(var v in dt[f])if(s.smSuxConfig[f][0].in"
+"dexOf(':')!= -1){vv[dt[f][v]]=s.smSuxConfig[f][0];}else{vv[dt[f][v]]"
+"=s.smSuxConfig[f][0]+':'+s.smSuxRules[v][1];}}vv=s._sa(vv);var j=0,k"
...
s.visitorNamespace="amfam"
s.trackingServer="metrics.amfam.com"
s.trackingServerSecure="smetrics.amfam.com"
s.dc="112"
