var QRF = {
		 ...
		 * Last Modified: September 24, 2013 1:57 PM
		 *
		...
		save_data : function (direction){
			...
			var mya = 0;
			console.log("line save_data 160: "+id);
			switch (id){
			case 'first':
		...
		slide : function(currentScreen, moveScreen, slideScreen){
			...
			//Added by Jon Toshmatov on 11/19/2013 at 7:50 AM
			var slide = $('.active').attr('name');
			if (QRF.host=='qrf.amfam.com' || QRF.host=='localhost'){
				if (slide!='select' && slide!='meetyouragent'){
					s.t();
					console.log("line slide func 238: "+slide);
				}
		...
		meet_your_agent : function (){
			...
			//data['agentaddress'] = $("#agentlist .sidebarAgentDivFocus .sidebarAddress").html();
			...
			var longlat2 = "http://maps.googleapis.com/maps/api/staticmap?size=435x350&maptype=roadmap\&markers=size:mid%7Ccolor:red%7C"+agentaddress+"&sensor=false";
		...
		update_omniture_tags : function (slide){
			...
			if (slide=="first"){
				QRF.get_products_for_omniture('first');
			...
			if(slide=="tell-us-about-yourself"){
				var products = QRF.get_products_for_omniture(slide);
				s.pageName="QRF:Tell Us About Yourself";
				...
				$.ajax({
					...
					$(xml).find('agent').each(function(){
						...
						if (agentemail==email){
							...
							html += "<li><a href='tel:"+agentphone.toLowerCase().replace('office','')+"'>"+agentphone.toLowerCase().replace('office','')+"</a></li>";
							// html += "<li class=''><a target='blank' href=\"mailto:"+agentemail+"\">Email</a><br /></li>";
							// html += "<li class=''><a target='blank' href=\""+agentInternetAddress+"\">Website</a></li>";
							$("#meetyouragent .agent-location").html(html);
...
$(document).ready(function(){
		...
		//QRF.referrer = document.location
...
/*
		if (QRF.domain.indexOf('af1dev.com')>=0){
			QRF.is_from_agent = QRF.referrer.indexOf('af1dev.com'); //greater than 1 is true <-- dev test
		}

		if (QRF.domain.indexOf('localhost')>=0){
			QRF.is_from_agent = QRF.referrer.indexOf('localhost'); //greater than 1 is true  -> localhost test
		}

		if (QRF.domain.indexOf('insurance-agency.amfam.com')>=0){
			QRF.is_from_agent = QRF.referrer.indexOf('insurance-agency.amfam.com'); //greater than 1 is true  -> localhost test
		}
		...
		*/
		...
		var is_zip_ran =0;
		var is_callme = false;
		  $("#callme").live('change', function(e){
		  	if($(this).is(':checked')){
		  		$("#bestimetocall").fadeIn();
				  $("#ProspectPhoneNumber").attr("required","1");
				  $("#ProspectPhoneNumber").closest("li").addClass("required");
				  is_callme = true;
		  	} else {
		  ...
		  $('.paging a, .btn-get_started a').live('click', function(e){
		  ...
		  $('.paging a, .btn-get_started a, #submit_qrf, .paging-buttons a').live('click', function(e){
			...
			if ($("#findyouragent").length>0 && current_fieldset_name=='findyouragent'){
				...
				if ($("#agentlist .sidebarAgentDivFocus").text().length>0 && QRF.slide_direction==1){
					QRF.meet_your_agent();
					//QRF.save_tell_us(); commented out by Jon Toshmatov 11/19/2013
				}
				if ($("#agentlist .sidebarAgentDivFocus").text().length<=0 && QRF.slide_direction==1){
			...
			if (next_fieldset_name=='thankyou'){
				...
				if (QRF.is_facebook==true){
					$("#agentinformation").html(QRF.agentinformationfacebook);
				}else{
					$("#agentinformation").html(name+address+phone+agentemail);
				}
				if (QRF.is_from_agent>0){
					$("#agentinformation").html(QRF.agentinformationsite);
				}
		...
		$("#different_agent").live('click', function(e){
			...
			if ($("#findyouragent").length>0 && QRF.ins_before=="meetyouragent"){
				//$("<fieldset name='findyouragent' class='screen' id='findyouragent'></fieldset>").insertBefore("#meetyouragent");09/06/13 JT
				//$("#findyouragent").load("findyouragent?zipcode="+zipcode); 09/06/13 JT
